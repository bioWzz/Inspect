                        ...
                        probs=c(constant=.5,sigmoid=.3,impulse=.2), na.rm=TRUE, seed=NULL)
{
    ...
    # distribution SUBJECT are known.
  {
      ...
      # the mean and the standard deviation of distribution OBJECT
    {
      ...
      idx <- .which.quantile(values=dist_subject, na.rm=TRUE, 
                             quantiles=quantiles)
  ...
  sampleNorm2DQuantile <- function(values_subject1, values_subject2, 
                                   dist_subject1, dist_subject2, dist_object, na.rm=TRUE, quantiles=10)
